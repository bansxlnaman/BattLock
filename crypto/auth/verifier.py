...
def verify_challenge_response(certificate, challenge, signature):

    public_key = deserialize_public_key(certificate.public_key)

    challenge_data = challenge.nonce + str(challenge.timestamp).encode()

    return verify_signature(public_key, challenge_data, signature)
