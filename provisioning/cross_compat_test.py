...
Proves the Python crypto layer and the physical ATECC608B are
interchangeable:

    Test A:  chip signs a nonce, Python verifies it.
    Test B:  Python signs the nonce, the chip verifies it.
    Test C:  a tampered signature is rejected by the chip.

Run after flashing hardware/atecc_bridge.ino and provisioning the chip.
...
# Fixed challenge so both sides sign the exact same bytes.
...
def main():
    ...
    sig_sw = sign_message_raw(priv, TEST_NONCE)
    ...
    chip.close()
    print(f"\n{passed}/3 PASSED — hardware and Python crypto agree.")
